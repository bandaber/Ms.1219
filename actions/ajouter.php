...
try {
	$query = "INSERT INTO remarques SET reponse = :reponse, position = :position, sujet = :sujet, auteur = :auteur, texte = :texte, notes = :notes, images = :images, fichiers = :fichiers, date = :date";
	...
	if ($stmt->execute()){
		echo "Remaque ajoutée à la base de données";
	} else {
		echo "Impossible d'ajouter la remarque";
	}

	$id_base = $con->lastInsertId(); 
